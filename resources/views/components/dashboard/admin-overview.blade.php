...
<section class="space-y-5">
    <div class="flex flex-wrap items-end justify-between gap-4">
        ...
        <form method="GET" action="{{ route('dashboard') }}" class="flex flex-wrap items-end gap-2" x-data
            ...
            {{-- Last in the row, because the period, division and section it
                 carries are chosen in the three selects before it. It used to
                 sit on a line of its own above them, which put a button in
                 the middle of the controls that answer it. --}}
            <a href="{{ route(
                'admin.ipcrs.index',
                array_filter([
                ...
                class="rounded-lg bg-nav-900 px-4 py-2 text-xs font-semibold text-white transition-colors hover:bg-nav-800">
                Manage IPCRs
            </a>
        </form>
    </div>
    ...
    <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
        ...
        <x-dashboard.kpi-card label="Draft" :value="$t['draft']" sub="not yet submitted" accent="amber" :percent="$pct($t['draft'])">
            <x-slot:icon>
                <svg class="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                    <path stroke-linecap="round"
        ...
        <x-dashboard.kpi-card label="For Final Approval" :value="$t['final']" sub="waiting for final approval"
            ...
            <x-slot:icon>
                <svg class="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                    <path stroke-linecap="round"
                        d="M12 3l2.5 5.5 6 .8-4.4 4.2 1.1 6-5.2-2.9L6.8 19.5l1.1-6L3.5 9.3l6-.8z" />
                </svg>
            </x-slot:icon>
        </x-dashboard.kpi-card>

        <x-dashboard.kpi-card label="Approved" :value="$t['approved']" sub="finalized" accent="green" :percent="$pct($t['approved'])">
            <x-slot:icon>
                <svg class="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
    ...
    {{-- Everything from here down runs beside the rail.

         The headline above it - the filters, the period tabs, the five
         figures - reads across the whole page, because that is the shape of a
         strip of numbers. What follows is a column of panels, and a column
        ...
        <div class="min-w-0 space-y-5">
            ...
            <div class="grid gap-4 lg:grid-cols-2">
                ...
                <div class="rounded-xl bg-white p-5 shadow-sm ring-1 ring-gray-950/5">
                    ...
                    <div class="mt-3 grid grid-cols-2 gap-2">
                        ...
                        <div class="rounded-lg bg-gray-50 p-3 text-center">
                            <p class="font-data text-2xl font-semibold text-gray-900">{{ $t['section_head_track'] }}
                            </p>
                    ...
                    <div class="mt-4 border-t border-gray-100 pt-4">
                        ...
                        <div class="mt-2 h-1.5 overflow-hidden rounded-full bg-gray-100">
                            <div class="h-full rounded-full bg-emerald-600" style="width: {{ $approvalRate }}%">
                            </div>
                        </div>
                    </div>
                </div>
            ...
            @if ($admin['divisionStats'] !== [])
                <div class="rounded-xl bg-white shadow-sm ring-1 ring-gray-950/5">
                    <div class="border-b border-gray-100 px-5 py-4">
                        ...
                        <p class="mt-0.5 text-xs text-gray-500">Select a row to narrow everything above to that
                            division.</p>
                    </div>
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full text-sm">
                            <thead>
                                <tr
                                    class="border-b border-gray-100 text-[0.6875rem] uppercase tracking-wide text-gray-500">
                            ...
                            <tbody>
                                @foreach ($admin['divisionStats'] as $row)
                                    @php
                                        ...
                                        $rowPct =
                                            $row['total'] > 0
                                                ? (int) round(($row['approved'] / $row['total']) * 100)
                                                : 0;
                                        $target = $isActive
                                            ? $scope->toQuery([
                                            ...
                                            : $scope->toQuery([
                                                'filter_division_id' => $row['id'],
                                                'filter_section_id' => null,
                                            ]);
                                    @endphp
                                    <tr
                                        class="border-b border-gray-50 last:border-0 {{ $isActive ? 'bg-sky-50/70' : 'hover:bg-gray-50' }}">
                                        <td
                                            class="px-5 py-2.5 font-medium text-gray-900 {{ $isActive ? 'border-s-2 border-sky-500 ps-4' : '' }}">
                                        ...
                                        <td class="px-3 py-2.5 text-center font-data font-medium text-emerald-700">
                                            {{ $row['approved'] }}</td>
                                        <td class="px-3 py-2.5 text-center font-data text-sky-700">
                                            {{ $row['review'] }}</td>
                                        <td class="px-3 py-2.5 text-center font-data text-teal-700">
                                            {{ $row['final'] }}</td>
                                        <td class="px-3 py-2.5 text-center font-data text-gray-400">
                                            {{ $row['draft'] }}</td>
                                        <td class="px-3 py-2.5 text-center font-data text-gray-700">
                                        ...
                                        <td class="px-3 py-2.5">
                                            <div class="flex items-center justify-center gap-2">
                                                ...
                                                <span
                                                    class="font-data text-[0.6875rem] text-gray-500">{{ $rowPct }}%</span>
                                            </div>
                                        </td>
                                    </tr>
                                @endforeach
            ...
            <div class="rounded-xl bg-white shadow-sm ring-1 ring-gray-950/5">
                <div class="border-b border-gray-100 px-5 py-4">
                    ...
                    <p class="mt-0.5 text-xs text-gray-500">Active employees with no IPCR sent for assessment. A draft
                        does
                        not count.</p>
                </div>
                ...
                @elseif ($admin['notSubmitted']->isEmpty())
                    <p
                        class="m-5 rounded-lg bg-emerald-50 px-4 py-3 text-sm text-emerald-800 ring-1 ring-emerald-500/20">
                ...
                @else
                    <p class="m-5 rounded-lg bg-amber-50 px-4 py-3 text-sm text-amber-900 ring-1 ring-amber-500/20">
                        ...
                        {{ $admin['notSubmitted']->count() === 1 ? 'employee has' : 'employees have' }} not submitted
                        in this
                        scope.
                    </p>
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full text-sm">
                            <thead>
                                <tr
                                    class="border-y border-gray-100 text-[0.6875rem] uppercase tracking-wide text-gray-500">
                                ...
                                @foreach ($admin['notSubmitted'] as $person)
                                    <tr class="border-b border-gray-50 last:border-0">
                                        <td class="px-5 py-2.5">
                                            <span
                                                class="block font-medium text-gray-900">{{ $person->full_name }}</span>
                                            @if ($person->user)
                                                <span
                                        ...
                                        <td class="px-5 py-2.5 text-xs text-gray-500">
                                            {{ $person->section?->division?->name ?? ($person->division?->name ?? '—') }}
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
